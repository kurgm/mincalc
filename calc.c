...
#define CALC_FUNDEF_BUFSIZE 16384
...
int do_svar(const symb_t *symb, const char *input) {
...
#ifndef NDEBUG
        if (id->type != -1) CALC_DIE("assignment lhs not a token");
    ...
    if (symb->type == RL_SETVAR_FUNDEF) {
        ...
        const symb_t *id = symb->arg1->arg1;
#ifndef NDEBUG
        if (id->type != -1) CALC_DIE("fundef lhs id not a token");
...
int do_eval_ctx(int *result, const symb_t *symb, var_entry_t *ctx,
    ...
    switch (symb->type) {
        case -1:
            if (symb->token.type == TOK_NUM) {
                *result = symb->token.num;
                return 0;
            }
            if (symb->token.type == TOK_ID) {
                var_entry_t *e;
        ...
        case RL_FUNCALL: {
#ifndef NDEBUG
            if (symb->arg1->type != -1) CALC_DIE("funcall id not a token");
...
int call_function(int *result, const char *fundef_str, symb_t *arglist_opt,
    ...
    if (argc != 0) {
        ...
        while (arg->type == RL_ARGLIST_CONS) {
#ifndef NDEBUG
            if (id->arg1->type != -1) CALC_DIE("idlist id not token");
...
#ifndef NDEBUG
        if (id->arg1->type != -1) CALC_DIE("idlist id not token 2");
